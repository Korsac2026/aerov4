...
local LOADER_URL = "https://raw.githubusercontent.com/uranium-client/uranium/main/downloader.lua" 
local _initArgs = ...
if type(_initArgs) ~= "table" then _initArgs = {} end
shared.uraniumUser = "user"
...
local function migrateProfiles()
	if isfile('uranium/profiles/migrated_placeid.txt') then return end

	local oldId = tostring(game.GameId)
	local newId = tostring(game.PlaceId)

	if oldId == newId then
		pcall(writefile, 'uranium/profiles/migrated_placeid.txt', 'done')
		return
	end

	local suffix = oldId .. '.txt'
	for _, path in ipairs(listfiles('uranium/profiles')) do
	...
	if isfolder('uranium/profiles/premade') then
		for _, path in ipairs(listfiles('uranium/profiles/premade')) do
			local name = path:gsub('\\', '/')
			if name:sub(-#suffix) == suffix then
				local newPath = name:sub(1, -#suffix - 1) .. newId .. '.txt'
				if not isfile(newPath) then
...
local function finishLoading()
	...
	if not vape.Load then
		warn('[uranium] vape.Load is nil skipping load')
		return
	end
	vape:Load()
	...
	if not shared.vapereload then
		...
		if vape.Categories.Main.Options['GUI bind indicator'].Enabled then
			vape:CreateNotification('[uranium] Finished Loading', 'Welcome ' .. shared.uraniumUser .. ' ' .. (vape.VapeButton and 'Press the button in the top right to open GUI' or 'Press ' .. table.concat(vape.Keybind, ' + '):upper() .. ' to open GUI'), 5)
		end
	end
end

if not isfile('uranium/profiles/gui.txt') then
	writefile('uranium/profiles/gui.txt', 'new')
end
local gui = readfile('uranium/profiles/gui.txt')

if not isfolder('uranium/assets/' .. gui) then
	makefolder('uranium/assets/' .. gui)
end

local guiSource = downloadFile('uranium/guis/' .. gui .. '.lua')
...
if not guiFunc then
	...
	error('[uranium] syntax error in ' .. gui .. '.lua' .. '\n' .. errMsg .. context)
end
vape = guiFunc()
if not vape then
	error('[uranium] GUI returned nil file may be corrupted try deleting uranium/guis/' .. gui .. '.lua and reinjecting.')
end
if not vape.Load then
	if delfile then pcall(function() delfile('uranium/guis/' .. gui .. '.lua') end) end
	error('[uranium] gui file corrupted (missing load) reinject..')
end
if not vape.Init and not vape.Load then
	error('[uranium] failed to initialize properly reinject to fix this bs')
end
shared.vape = vape
task.wait(0.1)
...
if not shared.VapeIndependent then
	loadstring(downloadFile('uranium/games/universal.lua'), 'universal')()
	local gameFileId = (game.GameId == 2619619496) and (game.PlaceId == 6872265039 and 6872265039 or 6872274481) or game.PlaceId

	if isfile('uranium/games/' .. gameFileId .. '.lua') then
		local gameSrc = downloadFile('uranium/games/' .. gameFileId .. '.lua')
		...
		if not gameFunc then
			...
			error('[uranium] syntax error in ' .. gameFileId .. '.lua\n' .. msg .. ctx)
		end
		gameFunc(...)
	else
		if not shared.VapeDeveloper then
			local suc, res = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/uranium-client/uranium/' .. readfile('uranium/profiles/commit.txt') .. '/games/' .. gameFileId .. '.lua', true)
			end)
			if suc and res and res ~= '404: Not Found' then
				local path = 'uranium/games/' .. gameFileId .. '.lua'
				if not isfile(path) then
					writefile(path, '--This watermark is used to delete the file if its cached, remove it to make the file persist after uranium updates.\n' .. res)
				end
